  ...
  Future<LineChartBarData> get lineChartBarData1_1 async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < getAnalytic.beatAvgs!.length; i++) {
        ...
        spots[month - 1] = FlSpot(month.toDouble(), getAnalytic.beatAvgs![i].avg![0]);
      }
    } else {
...
//spo2 1
  ...
  Future<LineChartBarData> get lineChartBarData1_3 async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < getAnalytic.beatAvgs!.length; i++) {
        ...
        List<String> parts = monthString.split('/');
        int month = int.parse(parts[0]);
        spots[month - 1] = FlSpot(month.toDouble(), getAnalytic.beatAvgs![i].avg![2]);
      }
    } else {
  ...
  Future<LineChartBarData> get lineChartBarData2_1 async {
    ...
    return LineChartBarData(
      ...
      isStrokeCapRound: true,
      dotData: FlDotData(show: false),
      belowBarData: BarAreaData(show: true,color: Colors.red.withOpacity(0.8),),
      spots: spots,
    );
  }
...
class LineChartSample1State extends State<HeartBeat> {
